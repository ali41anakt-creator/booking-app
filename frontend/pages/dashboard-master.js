...
async function loadMasterSchedule() {
  ...
  try {
    ...
    el.innerHTML = slots.map(s => `
      <div class="slot-btn" style="cursor:default;${s.available?'':'opacity:0.4;background:rgba(192,57,43,0.1);border-color:rgba(192,57,43,0.3)'}">
        ${s.time}<br><span style="font-size:0.7rem">${s.available?'🟢':'🔴'}</span>
      </div>`).join('');
