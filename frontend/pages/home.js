...
async function renderHomePage() {
  ...
  <main class="page">
    ...
    <section class="section" style="background:var(--surface)">
      <div class="container">
        ...
        <div class="services-filter-bar" style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:24px">
          <input
            type="text"
            id="service-search"
            placeholder="Қызметті іздеу (мысалы: маникюр)..."
            style="flex:1;min-width:220px;padding:10px 14px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:inherit"
          >
          <select id="service-category" style="padding:10px 14px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:inherit">
  ...
  // Load services (with search/filter support)
  function renderServices(services) {
    ...
    grid.innerHTML = services.length
      ...
      : '<p style="color:var(--gray-lt)">Сәйкес қызмет табылмады</p>';
  }
  ...
  try {
    const categories = await api.get('/services/meta/categories');
    const select = document.getElementById('service-category');
    select.innerHTML = '<option value="">Барлық санаттар</option>' +
      categories.map(c => `<option value="${c}">${c}</option>`).join('');
  } catch { /* категориялар жүктелмесе, фильтр сүзгісіз қалады */ }
