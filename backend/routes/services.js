...
// GET /api/services?search=...&category=... — public, поиск/фильтрация
...
// GET /api/services/meta/categories — қолжетімді категориялар тізімі (фильтр үшін)
router.get('/meta/categories', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT DISTINCT category FROM services WHERE is_active=TRUE AND category IS NOT NULL ORDER BY category`
    );
